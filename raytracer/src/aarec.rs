...
use crate::{Ray, Vec3, aabb::{AABB, fmax, fmin}, camera::degrees_to_radians, hittable::{self, Hittable, hit_record}, materia::{self, material, metal}};
...
impl Hittable for translate{
    ...
    fn bounding_box(&self, time0: f64, time1: f64) -> Option<AABB> {
        if let Some(output_box) = self.ptr.bounding_box(time0, time1){
            return Some(AABB::new(output_box.minmum +  self.offset,output_box.maxmum +  self.offset));
        }
        None
    }
