...
pub use vec3::Vec3;
pub use ray::Ray;
use crate::{camera::Camera, hittable::Hittable, hittable_list::Hittable_list, materia::{dielectric, lambertian, metal}, rtweekend::random_double, sphere::Sphere};
...
pub fn random_scene(world:&mut Hittable_list){
    ...
    for a in -11..11{
        for b in -11..11{
            ...
            let center = Vec3::new(a as f64 + 0.9 * random_double(1.0, 100.0),0.2,b as f64 + 0.9 *  random_double(1.0, 100.0));
            if (center - Vec3::new(4.0, 0.2, 0.0)).len() > 0.9{
                if choose_mat < 0.8{
                    let albedo1 = Vec3::random();
                    let albedo2 = Vec3::random();
                    let albedo = Vec3::new(albedo1.x * albedo2.x, albedo1.y * albedo2.y, albedo1.z * albedo2.z);
...
pub fn color(r:&Ray,world:&dyn Hittable,depth:i32) -> Vec3{
    ...
    if let Some(rec_) = t{
        ...
        if rec_.mat_ptr.scatter(r, &rec_, &mut attenuation, &mut scattered) {
            //println!("{},{},{}\n",attenuation.x,attenuation.y,attenuation.z);
            //return Vec3::cross(color(&scattered, world, depth - 1),attenuation);
            let t = color(&scattered, world, depth - 1);
            return Vec3::new(t.x * attenuation.x, t.y * attenuation.y , t.z * attenuation.z);
